...
**Next up (Phase 7.5, not started):** the business referral/network system — referrals are admin-entered records with an approval workflow, but no owner-invitation flow, no duplicate-business detection, no separate business verification step, and no per-referral negotiated commission (only a fixed platform-wide network rate).
