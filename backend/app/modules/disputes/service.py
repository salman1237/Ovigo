...
_EAGER = (selectinload(Dispute.raised_by),)


async def create_dispute(db: AsyncSession, user: User, payload: DisputeCreate) -> Dispute:
    result = await db.execute(
        select(Booking).where(Booking.id == payload.booking_id, Booking.user_id == user.id)
    )
...
async def resolve_dispute(db: AsyncSession, admin: User, dispute_id: uuid.UUID, payload: DisputeResolve) -> Dispute:
    ...
    await notifications_service.notify(
        ...
        message=f"Your dispute has been resolved ({payload.resolution.value}): {payload.note}",
        link=f"/bookings/{dispute.booking_id}",
    )
